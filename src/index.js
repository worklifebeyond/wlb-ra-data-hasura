...
module.exports = (serverEndpoint, headers, config) => {
    ...
    const getNewProperties = (prevObj, newObj) => {
        const prevObjProperties = Object.keys(prevObj);
        const newObjProperties = Object.keys(newObj);
        return newObjProperties.filter(prop => prevObjProperties.indexOf(prop) === -1);
    };
    ...
    const convertDataRequestToHTTP = (type, resource, params) => {
        ...
        if (params.data) {
            ...
            const reads = keys.filter(k => k.includes("__read"));
            reads.map(r => {
                delete params.data[r];
            })
        }
        ...
        switch (type) {
            ...
            case 'UPDATE':
                ...
                Object.keys(params.data).map((keyName, keyIndex) => {
                    if (params.previousData[keyName] !== params.data[keyName]) {
                        updatedData[keyName] = params.data[keyName];
                    }
                });
                ...
                finalQuery.args['$set'] = updatedData;
                finalQuery.args.where = {};
                finalQuery.args.where[primaryKey] = { '$eq': params.id };
            ...
            case 'UPDATE_MANY':
                ...
                Object.keys(params.data).map((keyName, keyIndex) => {
                    if (params.previousData[keyName] !== params.data[keyName]) {
                        updatedManyData[keyName] = params.data[keyName];
                    }
                });
                ...
                finalQuery.args['$set'] = updatedManyData;
                finalQuery.args.where = {};
    ...
    return (type, resource, params) => {
        ...
        return fetch(serverEndpoint + '/v1/query', options)
